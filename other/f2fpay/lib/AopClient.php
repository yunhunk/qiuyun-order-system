...
class AopClient
{
    ...
    protected function curl($url, $postFields = null)
    {
        ...
        //$this->writeLog("请求url：" . $url . "\n请求参数：" . (is_array($postFields) ? json_encode($postFields) : $postFields));
        ...
        if (curl_errno($ch)) {
            if (empty($reponse)) {
                throw new Exception("于" . (10 * 1000) . "毫秒后请求超时，未返回任何数据，请检查防火墙！", 502);
            } else {
    ...
    protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt)
    {
        ...
        $this->writeLog(json_encode($logData));
    }
