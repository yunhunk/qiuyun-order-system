...
if (preg_match('/Baiduspider/', $_SERVER['HTTP_USER_AGENT'])) {
    exit;
}
...
if ($conf['alipay_api'] != 5 && $conf['qqpay_api'] != 5 && $conf['wxpay_api'] != 5) {
    codepayMsg('{"error":"not open"}');
}

if (empty($conf['codepay_id']) || empty($conf['codepay_key'])) {
    codepayMsg('{"error":"not key"}');
}
...
if ($arr && $arr['code'] == 1) {
    ...
    $api_data['trade_no'] = '';
    $api_data['count']    = 0;
    if (!$arr['nonce_str'] || md5($arr['nonce_str'] . $conf['codepay_key']) != $arr['sign']) {
        codepayMsg('{"error":"fail"}');
    }
    ...
    foreach ($arr['data'] as $row) {
        if ($row['status'] == 1 && $count < $limit) {
            $count += 1;
            $out_trade_no = addslashes($row['param']);
            if (!$out_trade_no) {
                continue;
            }

            $srow = $DB->get_row("SELECT * FROM cmy_pay WHERE trade_no='{$out_trade_no}' limit 1 for update");
            if ($srow && $srow['status'] == 0) {
                $DB->query("update `pre_pay` set `status` ='1',`endtime` ='$date' where `trade_no`='{$out_trade_no}'");
