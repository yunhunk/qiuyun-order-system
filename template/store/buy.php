...
if (isset($price_obj)) {
    ...
    $price_1  = $price_obj->getToolCost($tool['tid'], 1);
    $price_2  = $price_obj->getToolCost2($tool['tid'], 1);
    $isLogin2 = $islogin3;
} else {
    $price    = $tool['price'];
    $price_pt = $tool['price'];
...
$isfaka = 0;
if ($tool['is_curl'] == 4) {
    $count         = intval($DB->count("SELECT count(*) FROM cmy_faka WHERE `tid`='" . $tool['tid'] . "' AND `orderid`=0 AND `status`=0"));
    $fakainput     = getFakaInput();
    $tool['input'] = $fakainput;
    $isfaka        = 1;
    if ($count <= 0) {
        $stock = '<span class="stock">剩余:<span class="quota" style="color:red">' . $count . '</span>份</span>';
    } else {
        $stock = '<span class="stock">剩余:<span class="quota">' . $count . '</span>份</span>';
    }
} elseif ($tool['stock_open'] == 1 && !is_null($tool['stock'])) {
    $count = $tool['stock'];
...
if (empty($tool['desc'])) {
    $tool['desc'] = '暂无较详细说明，如有问题请联系客服';
}

if ($tool['stock_open'] == 1) {
    if ($tool['stock'] > 30) {
        $stock_background = '#009688';
    } elseif ($tool['stock'] > 5) {
        $stock_background = '#00BCD4';
    } elseif ($tool['stock'] > 0) {
        $stock_background = '#FF9800';
    } else {
        $stock_background = '#FF5722';
    }
    if ($tool['stock'] > 0) {
        $stock = '剩' . $tool['stock'];
    } else {
...
$defaultImg = $weburl . 'assets/template/store/picture/error_img.png';

if ($tool['shopimg']) {
    if (!preg_match('/http/', $tool['shopimg'])) {
        $tool['shopimg'] = $weburl . $tool['shopimg'];
    }
} else {
    $tool['shopimg'] = $weburl . 'assets/template/store/picture/error_img.png';
}
...
<head>
    ...
    <title><?php echo isset($title) ? $title : $conf['title']; ?> - <?php echo $conf['sitename'] ?></title>
    ...
    <link rel="stylesheet" type="text/css" href="<?php echo $cdnserver; ?>assets/template/store/css/foxui.diy.css">
    <link rel="stylesheet" type="text/css" href="<?php echo $cdnserver; ?>assets/template/store/css/style(1).css">
    <link rel="stylesheet" type="text/css" href="<?php echo $cdnserver; ?>assets/template/store/css/iconfont.css">
    <link rel="stylesheet" type="text/css" href="<?php echo $cdnserver; ?>assets/template/store/css/detail.css">
    ...
    <link href="<?php echo $cdnpublic ?>Swiper/4.5.0/css/swiper.min.css" rel="stylesheet">
...
<style>
    ...
    * {
    ...
    　　-webkit-animation:hue 3s infinitelinear;
        animation:hue 3s infinitelinear;
    }
    @keyframes hue{
    　　from{
...
<div class="fui-page-group statusbar" style="max-width: 650px;left: auto;">
    <div class="fui-page  fui-page-current " style="overflow: inherit">
        <div id="container" class="fui-content "
            ...
            <div class="pro_content" style="margin-bottom: 4.2rem;">
                <div class="list_item_box" style="top: 53px;">
                    <div class="bor_detail">
                        <div class="thumb" id="layer-photos-demo" class="layer-photos-demo">
                            <img onerror="this.src='<?php echo $defaultImg; ?>'" alt="<?php echo $tool['name'] ?>" layer-src="<?php echo $tool['shopimg'] ?>" src="<?php echo $tool['shopimg'] ?>">
                        </div>
                        <div class="pro_right fl">
                            ...
                            <div class="list_tag">
                                <div class="price">
                                    <span class="t_price pay_prices">
                                        ￥<?php echo $price ?>元
                                       <?php if ($conf['template_showprice'] == 1 && (!$isLogin2 || $userrow['power'] < 2)) {?>
                                        &nbsp;&nbsp;<button type="button" class="show_daili_price layui-btn layui-btn-warm layui-btn-xs layui-btn-radius "><i class="layui-icon layui-icon-fire"></i>想要优惠</button>
                                        <?php }?>
                                    </span>
                                    <span class="sale" style="background-color: <?php echo $stock_background ?>;
    color: #fff;
    padding: 2px 5px;
    border-radius: 3px;"><?php echo $stock ?></span>
                                    <?php if ($conf['template_showsales'] == 1): ?>
                                        <span class="sale" style="background-color: #e563fb;
    color: #fff;
    padding: 2px 5px;
    border-radius: 3px;">已售<span style="margin:0 1px;"><?php echo $tool['sale'] ?></span> 件</span>
                                    <?php endif?>
                                </div>
                            </div>
                        </div>
            ...
            <span>
                 <a href="<?php echo $weburl; ?>user/login.php?back=<?php echo urlencode('?mod=buy&tid=' . $tid) ?>" style="color: salmon">登录后下单订单不丢失！点我即可登录~</a>
            </span>
            </p>
            <?php }?>
...
<input type="hidden" id="tid" value="<?php echo $tid ?>" cid="<?php echo $tool['cid'] ?>" price="<?php echo $price; ?>" alert="<?php echo escape($tool['alert']) ?>" inputname="<?php echo $tool['input'] ?>" inputsname="<?php echo $tool['inputs'] ?>" multi="<?php echo $tool['multi'] ?>" isfaka="<?php echo $isfaka ?>" count="<?php echo $tool['value'] ?>" active="<?php echo $tool['active'] ?>" prices="<?php echo $tool['prices'] ?>" max="<?php echo $tool['max'] ?>" min="<?php echo $tool['min'] ?>">
...
<script src="<?php echo $cdnpublic ?>Swiper/4.5.0/js/swiper.min.js"></script>
...
<div id="show_daili_price_html" style="display:none;">
    <div class="price" style="text-align:center;">
        <hr>
        <p class="pay_prices" id="level"><font color="#48d1cc">普通用户购价</font>：<span class="pay_price"><?php echo $price_pt ?>元</span></p>
        <p class="pay_prices" id="level"><font color="red">普通代理购价</font>：<span class="pay_price"><?php echo $price_1 ?>元</span></p>
        <p class="pay_prices" id="level"><font color="orange">高级代理购价</font>：<span class="pay_price"><?php echo $price_2 ?>元</span></p>
        <hr>
...
$(".show_daili_price").on("click",function(){
     layer.open({
          type: 1,
          title: "商品【<?php echo $tool['name'] ?>】等级优惠价格",
          btnAlign: 'c',
          ...
          yes: function(index, layero){
             window.location.href = "<?php echo $weburl; ?>user/regsite.php";
          },
          <?php }?>
     });
});
...
<script src="<?php echo $cdnserver ?>assets/template/store/js/main.js?<?php echo $jsver ?>"></script>
<?php echo @hook('footer_after') ?>
</body>
</html>