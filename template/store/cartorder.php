...
$row     = $DB->get_row("SELECT * from `pre_pay` where `trade_no`='{$orderid}' limit 1");
if (!$row) {
    sysmsg('当前订单不存在');
}
...
$share_link = '我钱不够买这个东西，能够帮我买一下嘛~，这是付款订单,谢谢啦 ' . $weburl . '?mod=cartorder&orderid=' . $orderid . '&set';
...
<head>
    ...
    <link href="<?php echo $cdnpublic ?>animate.css/3.5.2/animate.min.css" rel="stylesheet">
</head>
<style>
    .information {
        font-size: 0.9em;
                ...
                <div class="layui-card-body layui-form layui-form-pane" style="padding: 0.5em">
                    ...
                    <div style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;">
                        <input type="radio" name="pay" value="rmb"
                               title="<p style='width:100%'><img src='<?php echo $cdnserver ?>assets/img/rmb.png' width='13px' /> 余额 <span style='color:#999;font-size:0.8em;margin-left:1em'>剩<?php echo $userrow['money'] ?>元</span></p>">
                    </div>
                    <?php }?>
...
<script src="<?php echo $cdnserver ?>assets/template/store/js/cart.js?ver=<?php echo VERSION ?>"></script>
<script>
