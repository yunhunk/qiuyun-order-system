...
function display_zt($zt)
{
    if ($zt == 1) {
        ...
        return '<font color=grey>待退单</font>';
    } else {
...
if (!empty($data)) {
    ...
    if ($conf['query_checkcookie'] == 1 || $isLogin2 == 1) {
        $sql .= " AND A.`userid`='{$cookiesid}'";
    }
} else {
    $sql = " A.userid='{$cookiesid}'";
}

if (isset($_GET['status']) && $_GET['status'] != "") {
    $q_status = intval(input('get.status'));
    $sql .= " AND A.status = '{$q_status}'";
}
$limit = 10;
$start = $limit * ($page - 1);

$total      = $DB->get_column("SELECT count(*) FROM `pre_orders` A WHERE{$sql} ");
...
if (is_array($rs)) {
    foreach ($rs as $key => $res) {
        $record[] = array('id' => $res['id'], 'tid' => $res['tid'], 'input' => $res['input'], 'money' => $res['money'], 'name' => $res['name'], 'shopimg' => $res['shopimg'], 'value' => $res['value'], 'addtime' => $res['addtime'], 'endtime' => $res['endtime'], 'result' => $res['result'], 'status' => $res['status'], 'djzt' => $res['djzt'], 'skey' => getOrderSkey($res, 'get'));
    }
}
                    ...
                    <span class="layui-layout-right layui-elip" style="width:30%;text-align: right;margin-right: 0.5em">
                       ...
                       <div class="layui-row layui-col-space10">
                            <div class="layui-col-xs4">
                                ...
                                <img src="<?php echo $row['shopimg'] ?>" onerror="this.src='<?php echo $cdnserver ?>assets/template/store/picture/error_img.png'">
                                </a>
                            </div>
...
<div class="fui-content navbar order-list">
    <div class="fui-content-inner">
        <div class="content-empty" style="">
            <img src="<?php echo $weburl; ?>assets/template/store/picture/nolist.png" style="width: 6rem;margin-bottom: .5rem;"><br>
            <?php if ($_GET['data']) {?>
...
<script src="<?php echo $cdnserver ?>assets/template/store/js/query.js?v=<?php echo $jsver; ?>"></script>
