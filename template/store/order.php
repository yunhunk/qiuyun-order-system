...
$row     = $DB->get_row("SELECT * from cmy_pay where trade_no='$orderid' limit 1");
if (!$row) {
    sysmsg('当前订单不存在');
}
...
if (isset($_GET['set'])) {
    $show = '<font size="3" color="#f4a460">考虑到用户隐私问题，平台已经隐藏该用户下单信息</font>';
}
...
$share_link = '我钱不够买这个东西，能够帮我买一下嘛~，这是付款订单,谢谢啦 ' . $weburl . '?mod=order&orderid=' . $orderid . '&set';

$row_cate = $DB->get_row("SELECT * FROM `pre_class` where cid= ?", [$tool['cid']]);

$conf = getPayConf($tool, $class_info);

if (empty($tool['shopimg'])) {
    $tool['shopimg'] = $cdnserver . 'assets/template/store/picture/error_img.png';
}
                ...
                <div class="layui-row layui-col-space8">
                    <div class="layui-col-xs2">
                        <div style="border-radius: 0.5em;width: 3em;text-align: center;height: 3em;line-height: 3em;color:white;margin: 0.8em;box-shadow: 3px 3px 16px #eee">
                            <img src="<?php echo $cdnserver ?>assets/img/pays.png" width="35"/>
                        </div>
                    </div>
        ...
        <div class="layui-col-sm6" style="margin-top: 0.5em;">
            <div class="layui-card" style="border-radius: 0.5em">
                <div class="layui-card-header">
                    ...
                    <span style="float: right;"><?php echo $isfaka ? '<span class="layui-badge layui-bg-orange">卡密发货' : ($tool['is_curl'] == 2 || $tool['is_curl'] == 1 ? '<span class="layui-badge" style="background-color: lightsalmon">自动发货' : '<span class="layui-badge" style="background-color: red">手动发货') ?></span></span>
                </div>
                <div class="layui-row layui-col-space8">
                    <div class="layui-col-xs2">
                        <div style="border-radius: 0.5em;text-align: center;margin: 0.8em;box-shadow: 3px 3px 16px #eee" id="layer-photos-demo" class="layer-photos-demo">
                            <img alt="<?php echo $tool['name'] ?>" onerror="this.src='<?php echo $cdnserver ?>assets/template/store/picture/error_img.png'" layer-src="<?php echo $tool['shopimg'] ?>" src="<?php echo $tool['shopimg'] ?>" style="width: 100%;border-radius: 0.5em"/>
                        </div>
                    </div>
                    <div class="layui-col-xs10">
                        <div class="layui-card-header"
        ...
        <div class="layui-col-sm6" style="margin-top: 0.5em;">
            <div class="layui-card" style="border-radius: 0.5em">
                ...
                <div class="layui-card-body layui-form layui-form-pane" style="padding: 0.5em">
                    <?php if ($conf['rmbpay_api'] == 1) {?>
                    <div style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;">
                        <input type="radio" name="pay" value="rmb"
                               title="<p style='width:100%'><img src='<?php echo $cdnserver ?>assets/img/rmb.png' width='13px' /> 余额 <span style='color:#999;font-size:0.8em;margin-left:1em'>剩<?php echo round($userrow['money'], 2) ?>元</span></p>">
                    </div>
                    <?php }?>
                    ...
                    <div style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;">
                        <input type="radio" name="pay" <?php echo ($conf['alipay_api'] != 0 ? '' : 'disabled') ?>
                               value="alipay"
                               title="<p style='width:100%'><img src='<?php echo $cdnserver ?>assets/img/alipay.png' width='13px' /> 支付宝</p>">
                    </div>
                    <?php }?>
                    ...
                    <div style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;">
                        <input type="radio" name="pay" <?php echo ($conf['wxpay_api'] != 0 ? '' : 'disabled') ?> value="wxpay"
                               title="<p style='width:100%'><img src='<?php echo $cdnserver ?>assets/img/wxpay.png' width='13px' /> 微信</p>">
                    </div>
                    <?php }?>
                    ...
                    <div style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;">
                        <input type="radio" name="pay" <?php echo ($conf['qqpay_api'] != 0 ? '' : 'disabled') ?> value="qqpay"
                               title="<p style='width:100%'><img src='<?php echo $cdnserver ?>assets/img/qqpay.png' width='13px' /> QQ钱包</p>">
                    </div>
                    <?php }?>

                   <!--  <div id="demo_url" data-url="<?php echo $share_link ?>" style="padding: 0.3em;border-radius: 0.3em;line-height: 2em;height: 2.5em;display: none" >
                        <input type="radio" name="pay" value="help"
                               title="<p style='width:100%'><img src='<?php echo $cdnserver ?>assets/img/payd.png' width='13px' /> 帮我付 (朋友代付)</p>">
                    </div> -->
                </div>
            </div>
        </div>
...
<script src="<?php echo $cdnserver ?>assets/template/store/js/order.js?=<?php echo $jsver ?>"></script>
<script>
layer.photos({
