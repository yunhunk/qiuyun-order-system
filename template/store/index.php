...
if (isset($_GET['tid']) && !empty($_GET['tid'])) {
    $tid  = intval($_GET['tid']);
    $tool = $DB->get_row("SELECT tid from cmy_tools where tid='$tid' limit 1");
    if ($tool) {
...
$ar_data = [];
if ($conf['template_store_allbtn'] == 1) {
    $ar_data = [
        [
            'cid'  => 0,
            'name' => '所有商品',
...
$re        = $DB->select("SELECT * FROM `pre_class` WHERE `active` = 1 AND (`upcid` IS NULL OR upcid<1) ORDER BY `sort` ASC ");
$qcid      = "";
$cat_name  = "";
$ar_data   = [];
if ($re) {
    foreach ($re as $key => $res) {
        if ($is_fenzhan && in_array($res['cid'], $classhide)) {
            continue;
        } elseif ($res['islogin'] == 1 && !$isLogin2) {
            continue;
        }
        ...
        if ($res['shopimg']) {
            if (!preg_match('/http/', $res['shopimg'])) {
                $res['shopimg'] = $weburl . $res['shopimg'];
            }
        } else {
            $res['shopimg'] = $weburl . 'assets/img/Product/default.png';
        }
        $ar_data[] = $res;
    }
}

$class_show_num = $conf['index_class_num_style'] ? intval($conf['index_class_num_style']) : 2; //分类展示几组
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo $conf['sitename'] ?> - <?php echo isset($title) ? $title : $conf['title']; ?></title>
    ...
    <link rel="stylesheet" type="text/css"
        href="<?php echo $cdnserver; ?>assets/template/store/css/style.css?<?echo $jsver; ?>">
    <link rel="stylesheet" type="text/css" href="<?php echo $cdnserver; ?>assets/template/store/css/iconfont.css">
    <link rel="stylesheet" type="text/css" href="<?php echo $cdnserver; ?>assets/template/store/css/index.css">
    ...
    .fui-goods-group.block .fui-goods-item .image {
        ...
        <?php echo checkmobile() ? 'height: 4.5rem;' : 'height: 5.7rem;' ?>
    }
...
<body ontouchstart="" style="overflow: auto;height: auto !important;max-width: 650px;">
    <div id="body">
        <div style="position: fixed;    z-index: 100;    top: 30px;    left: 20px;       color: white;    padding: 2px 8px;      background-color: rgba(0,0,0,0.4);    border-radius: 5px;display: none"
            id="xn_text">
        </div>
...
?>
                        ...
                        <div class="fui-notice">
                            <div class="image">
                                <a href="JavaScript:void(0)" onclick="$('.tzgg').show()"><img
                                        src="<?php echo $cdnserver ?>assets/template/store/picture/1571065042489353.jpg"></a>
                            </div>
                            ...
                            <div class="fui-searchbar bar">
                                <div class="searchbar center searchbar-active" style="padding-right:2.5rem">
                                    ...
                                    <div class="search-input" style="border: 0px;padding-left:0px;padding-right:0px;">
                                        ...
                                        <input type="text" class="search" value="<?php echo input('get.kw'); ?>"
                                            name="kw" placeholder="输入商品关键字..." id="kw">
                                    </div>
                                </div>
                            </div>
                        </form>
...
foreach ($ar_data as $v) {
    ...
    echo '<a data-cid="' . $v['cid'] . '" data-name="' . $v['name'] . '" class="get_cat">
                                               <div class="mbg">
                                                   <p class="ico"><img src="' . $v['shopimg'] . '" onerror="this.src=\'' . $cdnserver . 'assets/template/store/picture/1562225141902335.jpg\'"></p>
...
if ($conf['template_showsales'] == 1) {
    $conf['template_sale'] == 1;
}
?>
                                </div>
                                <!-- Add Pagination -->
                                <div class="swiper-pagination"></div>
                                <?php if (intval($conf['template_store_classbtn'])): ?>
                    ...
                    <div class="goods_sort">
                        ...
                        <div class="item item-price" data-order="sale" data-sort="ASC"><span class="text">销量</span>
                            <span class="sorting">
                                <i class="icon icon-sanjiao2"></i>
                                <i class="icon icon-sanjiao1"></i>
                            </span>
                        ...
                        <div class="item"><span class="text"><a href="javascript:;"><i class="icon icon-sort"
                                        id="listblock" data-state="list" style="font-size:20px;"></i></a></span> </div>
                    </div>
                    <section style="text-align: center;display:none;height: 1.5rem;line-height: 1.6rem;"
                        class="show_class">
                        <section style="display: inline-block;" class="">

                            <section class="135brush" data-brushtype="text"
                                style="clear:both;margin:-18px 0px;text-align: center;color:#333;border-radius: 6px;padding:0px 1.5em;letter-spacing: 1.5px;">
                                <span style="color: #f79646;"><strong><span style="font-size: 15px;"><span
                                                class="catname_show">正在获取数据...</span></span></strong></span>
                            </section>
        ...
        <input type="hidden" name="_template_showsales" value="<?php echo intval($conf['template_showsales']) ?>">
        <input type="hidden" name="_template_store_classbtn"
            value="<?php echo intval($conf['template_store_classbtn']) ?>">
        ...
        <?php include TEMPLATE_ROOT . 'store/inc/nav.php';?>
    ...
    <script src="<?php echo $cdnpublic ?>Swiper/4.5.0/js/swiper.min.js?<?php echo $jsver ?>"></script>
    <script src="<?php echo $cdnserver ?>assets/template/store/js/foxui.js"></script>
    <script src="<?php echo $cdnserver ?>assets/template/store/js/layui.flow.js"></script>
    <script src="<?php echo $cdnserver ?>assets/template/store/js/index.js?<?php echo $jsver ?>"></script>
