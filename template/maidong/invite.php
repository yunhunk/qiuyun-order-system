...
if (!$conf['invite_tid']) {
    exit("<script language='javascript'>alert('当前站点未开启推广链接功能');window.location.href='./';</script>");
}

$invite_row = $DB->get_row("select * from pre_tools where tid='{$conf['invite_tid']}' limit 1");
if (!$invite_row) {
    exit("<script language='javascript'>alert('赠送商品ID不存在，请重新配置');window.location.href='./';</script>");
}
...
<div class="col-xs-12 col-sm-10 col-md-8 col-lg-5 center-block" style="float: none;">
    <div class="block">
        <div class="block-title">
			...
			<a href="<?php echo $cdnserver ?>user/reg.php" class="btn btn-danger btn-rounded" style="float:right;"><i class="fa fa-user-plus"></i>&nbsp;开通分站</a>
			</div>
        </div>
      </div>
    </div>
...
<script src="//cdn.staticfile.org/layer/3.4.0/layer.js"></script>
	...
	$('#submit_sub').click(function(){
		...
		$.ajax({
			...
			success : function (json)
			{
				layer.close(ii);
				if (json.code === 1) {
					 var value = '特价**0.1元起刷，免费领**，免费拉圈圈99+，**、**、**、空间访问、**双击、全民K歌、**视频，链接：' + json.url + ' (请复制链接到浏览器打开)';
                    $('#resulturl').html('<br><div class="list-group-item list-group-item-warning"><i class="fa fa-check-circle-o"></i>&nbsp;生成链接成功，请复制以下内容进行推广！</div><div class="col-xs-12 well well-sm">特价**0.1元起刷<br>免费领**，免费拉圈圈99+<br>**、**、**、空间访问、**双击、全民K歌、**视频<br><br>链接：' + json.url + '<br>(请复制链接到浏览器打开)</div><center><button class="btn btn-warning btn-sm" data-clipboard-text="'+json.url+'" id="copyurl">一键复制链接</button>&nbsp;<button class="btn btn-success btn-sm" data-clipboard-text="'+value+'" id="copycontent">一键复制广告语</button></center>');
				} else if (json.code === 2) {
					var value = '特价**0.1元起刷，免费领**，免费拉圈圈99+，**、**、**、空间访问、**双击、全民K歌、**视频，链接：' + json.url + '(请复制链接到浏览器打开)';
                    $('#resulturl').html('<br><div class="list-group-item list-group-item-warning"><i class="fa fa-info-circle"></i>&nbsp;您已生成过链接，请复制以下内容进行推广！</div><div class="col-xs-12 well well-sm">特价**0.1元起刷<br>免费领**，免费拉圈圈99+<br>**、**、**、空间访问、**双击、全民K歌、**视频、等等..<br>' + json.url + '<br>(请复制链接到浏览器打开)</div><center><button class="btn btn-warning btn-sm" data-clipboard-text="'+json.url+'" id="copyurl">一键复制链接</button>&nbsp;<button class="btn btn-success btn-sm" data-clipboard-text="'+value+'" id="copycontent">一键复制广告语</button></center>');
				} else {
