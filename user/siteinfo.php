...
<div class="col-md-6 col-sm-12">
	<div class="panel panel-default">
		...
		<?php if ($userrow['power'] > 0) {?>
			...
			<li style="font-weight:bold;overflow: hidden;" class="list-group-item">防红链接：<a href="javascript:;" id="copy-btn" data-clipboard-text="" >Loading...</a>&nbsp;&nbsp;&nbsp;<span class="pull-right"><button class="btn btn-default btn-xs" id="recreate_url">重新生成</button>&nbsp;&nbsp;<a href="javascript:void(0);" id="tips" class="btn btn-info btn-xs">说明</a></span></li>
			<?php }?>
			<li style="font-weight:bold" class="list-group-item">网站名称：<font color="blue"><?php echo $userrow['sitename'] ?></font></li>
			<li style="font-weight:bold" class="list-group-item">站点类型：<?php echo ($userrow['power'] == 2 ? '<font color=red>旗舰版</font>' : '<font color=red>专业版</font>') ?>&nbsp;<?php if ($conf['fenzhan_upgrade'] > 0 && $userrow['power'] == 1) {echo '<a href="upsite.php" class="btn btn-danger btn-xs pull-right">升级站点</a>';} else {echo '<a href="./sitelist.php" class="btn btn-danger btn-xs pull-right">下级管理</a>';}?></li>
			...
			<?php }?>
			<?php if ($conf['app_open'] == 1 && is_file('app.php')) {?><li style="font-weight:bold" class="list-group-item">客户端APP：<?php echo ($userrow['appurl'] ? '<a href="' . $userrow['appurl'] . '" target="_blank" style="color:#337ab7;">点击下载</a>' : '<font color="grey">未生成</font>'); ?><a href="./app.php" class="btn btn-warning btn-xs pull-right">在线生成</a></li><?php }?>
...
$(document).ready(function(){
	...
	$(document).on('click', '#tips', function(event) {
		layer.alert('防红链接：该链接可以在QQ直接打开的您的网站，方便推广！<br />Tips：点击短网址即可复制哦~',{icon: 3,title: '小提示',skin: 'layui-layer-molv layui-layer-wxd'});
	});
});
</script>
</body>
