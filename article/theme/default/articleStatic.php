...
if (isset($_GET['id'])) {
    if (!$active && file_exists($file_404)) {
        include $file_404;
        die;
    } else {
    ...
    $seotitle       = '平台资讯';
    $seokeywords    = '云小店论坛首页,云小店资讯,云小店公告,云小店资讯';
    $seodescription = '云小店论坛首页，可为您提供更好的服务，帮助大家解决各种问题';
}
?>
<!--
 # 站内通知前台版By 星河
...
<div class="panel panel-default">
  ...
  <div class="btn-group btn-group-justified">
    <div class="btn-group">
      <a class="btn btn-default" data-toggle="modal" href="<?php echo $weburl ?>"><font color="#ff0000"><i class="fa fa-home"></i> <span style="font-weight:bold">返回网站</span></font></a>
    </div>
    <div class="btn-group">
      <a class="btn btn-default" data-toggle="modal" href="<?php echo $weburl ?>article/index.html"><i class="fa fa-bars"></i> <span style="font-weight:bold">文章首页</span></a>
    </div>
    <div class="btn-group">
        <a class="btn btn-default" data-toggle="modal" href="<?php echo $weburl ?>user/regsite.php"><font color="#ff0000"><i class="fa fa-bolt"></i><span style="font-weight:bold">搭建分站</span></font></a>
    </div>
   </div>
</div>
<?php
...
if ($active === false) {
    ...
    ?>
              ...
              <div class="block" style="display: block;margin:8px auto;margin-bottom: 5px">
                 <?php if ($lasetId) {?>
                 <p>上一篇&nbsp;<a href="http://<?php echo $hosturl; ?>/article/?id=<?php echo $lasetId; ?>.html" title="<?php echo $lasetRow['title'] ?>"  style="color: blue"><?php echo $lasetRow['title']; ?></a></p>
                <?php } else {?>
                 <p>上一篇&nbsp;&nbsp;<a href="javascript:(0)" style="color: blue"><b>没有了</b></a></p>
                <?php }?>
                <?php if ($prevId) {?>
                <p>下一篇&nbsp;<a href="http://<?php echo $hosturl; ?>/article/?id=<?php echo $prevId; ?>.html" title="<?php echo $prevRow['title'] ?>"  style="color: blue"><?php echo $prevRow['title']; ?></a></p>
                <?php } else {?>
                <p>下一篇&nbsp;&nbsp;<a href="javascript:(0)" style="color: blue"><b>没有了</b></a></p>
                <?php }?>
            </div>
<!--content over-->
       </div>
...
<?php
    ...
    if ($_GET['kw'] && $conf['message_search'] == 1) {
        ...
        $msgcount = $DB->count("SELECT count(*) FROM pre_message WHERE type IN ($type) AND (`title` like ? OR content like ?) AND active=1", ['%' . $kw . '%', '%' . $kw . '%']);
        $rs       = $DB->query("SELECT * FROM pre_message WHERE type IN ({$type}) AND (`title` like ? OR content like ?) and active=1 ORDER BY top DESC,sort ASC,id DESC LIMIT {$offset},{$pagesize}", ['%' . $kw . '%', '%' . $kw . '%']);
        $link     = "&kw=" . $kw;
    } else {
...
foreach ($msgrow as $row) {
        echo '<tr class="widget animation-fadeInQuick onclick" onclick="window.location.href=\'http://' . $hosturl . '/article/' . $row['id'] . '.html\'"><td><a title="' . $row['title'] . '" href="http://' . $hosturl . '/article/' . $row['id'] . '.html"><b class="pull-left">' . $row['title'] . '</b><br/><small class="pull-right"><span class="text-muted">' . $row['addtime'] . '</span></small></a></td></tr>';
    }
    if ($msgcount == 0) {
   ...
   <?php if ($msgcount > $pagesize) {
        if ($page > 1) {
            echo '<a href="//' . $hosturl . '/article/index.html?page=' . ($page - 1) . $link . '" class="btn btn-default">上一页</a>';
        }
        if ($page < $pages) {
            echo '<a href="//' . $hosturl . '/article/index.html?page=' . ($page + 1) . $link . '" class="btn btn-default pull-right">下一页</a>';
        }
    }
    ?>
    </div>
