...
if ($my == 'getSkimg') {
    ...
    $row = $DB->get_row("SELECT * from pre_tixian where `id`='" . $id . "' limit 1");
    if ($row) {
        json_success('成功', $row);
    } else {
        json('该数据不存在 =>' . $id);
    ...
    $data = '<div class="panel panel-body">';
    ...
    $row    = $DB->get_row("SELECT * from pre_tixian where `id`='" . $id . "' limit 1");
    if ($row) {
        $update = Db::name('tixian')->where(['id' => $id])->update([
            'note' => $result,
        ]);
        if ($update !== false) {
            json_success('成功', $row);
    ...
    if ($row) {
        $bz = input('post.bz', 1);
        $DB->query("UPDATE pre_tixian SET status=2 where id='$id'");
        $row2 = $DB->get_row("SELECT * from pre_site where `zid`='" . $row['zid'] . "' limit 1");
        if (!$row2) {
            exit('{"code":-1,"msg":"该提现记录的所属站点不存在！"}');
        }

        if ($row['subtype'] == 1) {
            $sql = $DB->query("UPDATE pre_site SET `point`=`point`+{$row['money']} where zid='" . $row['zid'] . "'");
            $bz  = '提现已驳回到分站提成账户，当前提成账户' . ($row['zid'] + $row2['point']) . '元！' . ($bz != "" ? '驳回原因（' . $bz . '）' : null);
        } else {
            $sql = $DB->query("UPDATE pre_site SET `money`=`money`+{$row['money']} WHERE zid='" . $row['zid'] . "'");
            $bz  = '提现已驳回到分站提成账户，当前余额账户' . ($row['money'] + $row2['money']) . '元！' . ($bz != "" ? '驳回原因（' . $bz . '）' : null);
        }
        $sql2 = $DB->query("UPDATE pre_tixian SET `note`='{$bz}' where id='" . $id . "'");

        if ($sql !== false && $sql2 !== false) {
            addPointLogs($row['zid'], $row['money'], '驳回', $bz, null);
            json_success('驳回操作成功, 退回' . $row['money'] . '元');
        } else {
            json_error('驳回操作失败, ' . $DB->error() . '');
        }
    } else {
    ...
    $DB->query("UPDATE pre_tixian set `realmoney`=" . $realmoney . " where id='" . $id . "'");
    ...
    switch ($status) {
        // 处理中
        case 3:
            Db::name('tixian')->where(['id' => ['in', $ids]])->update(['status' => 3]);
            break;
        // 已完成
        case 1:
            Db::name('tixian')->where(['id' => ['in', $ids]])->update([
                'status'  => 1,
                'endtime' => $date,
            ]);
            break;
        // 待处理
        case 0:
            Db::name('tixian')->where(['id' => ['in', $ids]])->update(['status' => 0]);
            break;
        case 4:
            Db::name('tixian')->where(['id' => ['in', $ids]])->delete();
            break;
        default:
...
if ($conf['fenzhan_tixian'] == 0) {
    showmsg('当前站点未开放提现功能！<br/><a style="color: #6db5ef;" href="./set.php?mod=tixian">现在去开启</a>');
}
...
if ($my == 'delete') {
    ...
    $id = intval($_GET['id']);
    $DB->query("UPDATE pre_tixian set status=1,endtime=NOW() where id='$id'");
    exit("<script language='javascript'>alert('已变更为已提现状态');javascript:history.go(-1);</script>");
} elseif ($my == 'reset') {
    $id = intval($_GET['id']);
    $DB->query("UPDATE pre_tixian set status=0 where id='$id'");
    exit("<script language='javascript'>alert('已变更为待处理状态');javascript:history.go(-1);</script>");
} elseif ($my == 'handle') {
    $id = intval($_GET['id']);
    $DB->query("UPDATE pre_tixian set `status`=3 where id='$id'");
...
$numrows      = $DB->count("SELECT count(*) from `pre_tixian` WHERE {$sql}");
$countRmb     = round($DB->count("SELECT sum(realmoney) from `pre_tixian` WHERE status=1"), 2);
$countRmbNo   = round($DB->count("SELECT sum(realmoney) from `pre_tixian` WHERE status=0"), 2);
$countRmbBack = round($DB->count("SELECT sum(realmoney) from `pre_tixian` WHERE status=2"), 2);
$count5       = round($DB->count("SELECT sum(realmoney) from `pre_tixian` WHERE 1"), 2);

$count1 = round($DB->count("SELECT count(id) from `pre_tixian` WHERE status=1"), 2);
$count2 = round($DB->count("SELECT count(id) from `pre_tixian` WHERE status=0"), 2);
$count3 = round($DB->count("SELECT count(id) from `pre_tixian` WHERE status=2"), 2);
$count4 = round($DB->count("SELECT count(id) from `pre_tixian` WHERE status=3"), 2);
...
echo '
    ...
    <div class="block-title">
        <h3 class="panel-title">
            <form method="get" class="form-inline">
                ' . $title . '
                <a onclick="window.location.href=\'tixian.php\'" style="margin-left:5px;" class="btn btn-warning hidden-xs btn-xs">全部(' . $count0 . ')</a>
                <a onclick="window.location.href=\'tixian.php?status=0\'" style="margin-left:5px;" class="btn btn-primary hidden-xs btn-xs">待处理(' . $count2 . ')</a>
                <a onclick="window.location.href=\'tixian.php?status=3\'" style="margin-left:5px;margin-right:5px;" class="btn btn-info hidden-xs btn-xs">处理中(' . $count4 . ')</a>
                <a onclick="window.location.href=\'tixian.php?status=2\'" style="margin-left:5px;margin-right:5px;" class="btn btn-danger hidden-xs btn-xs">已驳回(' . $count3 . ')</a>
                <a onclick="window.location.href=\'tixian.php?status=1\'" style="margin-left:5px;margin-right:5px;" class="btn btn-success hidden-xs btn-xs">已完成(' . $count1 . ')</a>
            </form>
        </h3>
    </div>
    ...
    <form method="get" class="form-inline">
        ...
        <a href="./tixian.php" class="btn btn-warning">重置</a>
      ...
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th><input name="chkAll1" type="checkbox" id="chkAll1" />&nbsp;ID</th><th>ZID</th><th>金额</th><th>实际到账</th><th>提现方式</th><th>提现账号</th><th>姓名</th>' . ($conf['fenzhan_skimg'] == 1 ? '<th>收款图</th>' : null) . '<th>申请时间</th><th>完成时间</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>';
...
function inputInfo(id) {
    ...
    var area = [$(window).width() > 420 ? '420px' : '95%', 'auto'];
	$.ajax({
		type : 'GET',
		url : '?my=getTixian&id='+id,
		dataType : 'json',
...
function searchItem(){
    ...
    window.location.href="./tixian.php?kw=" + kw + '&type=' + type + '&status=' + status  ;
    return false;
}
    ...
    layer.open({
        ...
        success: function (index, layero) {
            function loadMoney(date) {
                $.ajax({
                    type: 'POST',
                    url: 'ajax.php?act=tixian_today_alipay',
                    data: {date},
                    dataType: 'json',
                    success: function (data) {
            ...
            $("#submit").click(function () {
                ...
                window.location.href = 'download_tixian.php?type=alipay&table=tixian&remark=' + $("input[name='remark']").val() + '&date=' + $("#myDate").val();
...
$(document).ready(function () {
    ...
    $(document).on('click', "#alipaycsv", function () {
        ...
        //     ids: ids
        // }
        // window.location.href ='./tixian.php?my=csv&'+ objectToQueryStringFixed(params);
    });
    selectRender();
});
