...
layui.config({}).extend({}).define(['laytpl', 'laypage', 'layer', 'form'], function(exports) {
    ...
    var $ = layui.$,
        ...
        //主模板
        ,
        TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}} {{d.data.id}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">', '{{# if(d.data.toolbar){ }}', '<div class="layui-table-tool"></div>', '{{# } }}', '<div class="layui-table-box">', '{{# var left, right; }}', '<div class="layui-table-header">', TPL_HEADER(), '</div>', '<div class="layui-table-filter">', TPL_FILTER(), '</div>', '<div class="layui-table-body layui-table-main">', TPL_BODY, '</div>', '{{# if(left){ }}', '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', TPL_HEADER({
                fixed: true
                ...
                fixed: 'right'
            }), '<div class="layui-table-mend"></div>', '</div>', '<div class="layui-table-body">', TPL_BODY, '</div>', '</div>', '{{# }; }}', '</div>', '{{# if(d.data.isPage){ }}', '<div class="layui-table-page">', '<div id="layui-table-page{{d.index}}"></div>', '</div>', '{{# } }}'
            /*,'<style>'
    ...
    Class.prototype.renderTdCss = function() {
        ...
        var styel = '<style id="' + id + '">' + function() {
            ...
            layui.each(that.config.cols, function(i1, item1) {
                layui.each(item1, function(i2, item2) {
                    ret += '.laytable-cell-' + that.index + '-' + (item2.field || i2) + '{' + 'width:' + (item2.width ? item2.width.toString().indexOf('px') >= 0 || item2.width == 'auto' ? item2.width : item2.width + "px" : "0px") + '}';
                });
            });
            return ret;
        }() + '</style>';
    ...
    Class.prototype.renderTd = function(obj, cols, numbers, i3) {
        ...
        var field = cols.field || i3,
            ...
            colStyle = cols.colStyle || '',
            classname = cols.class || '';
        var treeImgHtml = '';
        if (options.isTree) {
        ...
        if (classname) {
            var div_style = ' class="' + classname + '"';
        } else {
            var div_style = ' style="width: auto;height: 100%;' + (colStyle ? colStyle : '') + '"';
        }
        //td内容
        ...
        }() + '">' + treeImgHtml + '<div ' + div_style + '>' + function() {
