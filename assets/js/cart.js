...
function emptyCart() {
    var confirmobj = layer.confirm('你确实要清空购物车所有商品吗？', {
        ...
        $.ajax({
            type: "GET",
            url: serverPath + "ajax.php?act=cart_empty",
            dataType: 'json',
...
function cart_shop_edit(id) {
    ...
    $.ajax({
        type: "POST",
        url: serverPath + "ajax.php?act=cart_shop_item",
        data: {
            id: id
        },
        dataType: 'json',
...
function cart_shop_save(id) {
    ...
    $.ajax({
        type: "POST",
        url: serverPath + "ajax.php?act=pay&method=cart_edit",
        data: {
            shop_id: id,
                    ...
                    success: function(data) {
                        ...
                        if (data.code == 1) {
                            ...
                            var confirmobj = layer.confirm('你的余额不足，请充值！', {
                                ...
                                window.location.href = serverPath + 'user/#chongzhi';
                            }, function() {
                                layer.close(confirmobj);
                            });
                        } else if (data.code == -4) {
